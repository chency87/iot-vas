...
def init_exec_job(scheduler, **jobargs):
    ...
    if trigger_type == TRIGGER_BY_DATE:
        ...
        scheduler.add_job(func=func, id=id, kwargs={'cmd':jobargs,'task_id':id}, trigger='date' ,run_date=run_date, replace_existing=True)
        ...
        scheduler.add_job(func=func,id=id, kwargs={'cmd':jobargs,'task_id':id},trigger='cron',**cron_rel,replace_existing=True)
        return id
    else:
        pass
    ...
    try:
        ...
        for ret in ret_list:
            info = {
                ...
                'cmd':  ALLOWED_FUNC_NAME[ALLOWED_FUNC[int(ret.kwargs.get('cmd')['task_cmd']) % len(ALLOWED_FUNC)]] + '(' + ret.kwargs.get('cmd')['task_target_ip'] +')',
                'func':ret.func_ref,
...
def get_task_report_by_id(id):
    ...
    top = dashboard_top.format(host_dict['up'],'#hosts-service-tables', len(service_list),  '#', open_port_count, '#', device_count,'#')
...
def get_task_report_by_stdout(stdout):
    ...
    top = dashboard_top.format(nmap_report.hosts_up,'#', open_port_count, total_port_count, '#', len(port_list_counter), '#', os_count,'#')

    scan_info = dashboard_scaninfo.format(nmap_report.startedstr,nmap_report.scan_type,nmap_report.elapsed, nmap_report.hosts_total)
